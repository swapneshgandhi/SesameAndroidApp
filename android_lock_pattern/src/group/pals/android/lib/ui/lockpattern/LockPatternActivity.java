...
public class LockPatternActivity extends Activity {
	...
	public void onConfigurationChanged(Configuration newConfig) {
		Log.e(CLASSNAME, "onConfigurationChanged()");
	...
	private void doComparePattern(List<Cell> pattern) {
		...
		if (ACTION_COMPARE_PATTERN.equals(getIntent().getAction())) {
			...
			if (true){
				Log.e("doPatternCompare","calling db now");
				SQLiteDatabase.loadLibs(this);
				group.pals.android.sesame.database.SesameDbAdapter mDbHelperSA 
				=new group.pals.android.sesame.database.SesameDbAdapter(this);
				//compare if pattern is correct by trying to open the database with the pattern
				okey=mDbHelperSA.ComparePattern(LockPatternUtils
						.patternToSha1(pattern));
				Log.e("Compare",String.valueOf(okey)+ LockPatternUtils
						.patternToSha1(pattern));
			}
		...
		if (okey)
			finishWithResultOk(LockPatternUtils.patternToSha1(pattern).toCharArray());
		else {
			mRetryCount++;
	...
	private void doCheckAndCreatePattern(List<Cell> pattern) {
		...
		if (getIntent().hasExtra(EXTRA_PATTERN)) {
			...
			Log.e("lockcreate2",LockPatternUtils.patternToSha1(pattern));
			if (okey) {
			...
			Log.e("lockcreate1",LockPatternUtils.patternToSha1(pattern));
			getIntent().putExtra(
					EXTRA_PATTERN,
